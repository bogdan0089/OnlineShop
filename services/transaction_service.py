...
class TransactionService:
    ...
    @staticmethod
    async def get_my_transactions(current_client: Client, limit: int = 10, offset: int = 0) -> List[Transaction]:
        async with UnitOfWork() as uow:
            transactions = await uow.transaction.get_client_transactions(current_client.id, limit, offset)
            return transactions
